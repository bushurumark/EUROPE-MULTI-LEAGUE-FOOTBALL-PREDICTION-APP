...
def evaluate_model():
    # Filter a subset of data (let's say the last 100 rows) for evaluation purposes
    eval_data = data.tail(100)  # Adjust as necessary for the dataset size

    # Prepare the feature set and target variable (FTR is the target)
    X_eval = eval_data.drop(columns=['FTR', 'Date', 'HomeTeam', 'AwayTeam'])
    y_eval = eval_data['FTR'].map({'H': 1, 'D': 2, 'A': 3})
...
def main():
    ...
    # Button to evaluate the model's performance
    if st.button('Evaluate Model'):
        accuracy, cm_display = evaluate_model()
