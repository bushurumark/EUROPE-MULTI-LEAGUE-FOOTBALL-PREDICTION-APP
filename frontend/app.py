...
Original file is located at
    https://colab.research.google.com/drive/1_t1owrfv8gkOztaV0s6cIviDDWigvbaN
"""
...
# --- UI Config (unchanged from your original) ---
st.markdown("""
    <style>
    .stApp { background-color: #0000FF; }
    .title { color: #32CD32; text-align: center; font-size: 40px; }
    label { color: red !important; font-weight: bold; }
    .stSelectbox { color: #00008B; background-color: #ADD8E6; border-radius: 10px; border: 2px solid #32CD32; }
    .stButton button { background-color: #FFD700; color: black; font-size: 20px; border-radius: 10px; padding: 10px 20px; }
    .prediction-result { color: #FF4500; font-size: 30px; text-align: center; font-weight: bold; }
    </style>
...
# --- API Call ---
def call_api(home_team: str, away_team: str, league_type: str) -> Dict:
    API_URL = "https://football-prediction-app-btcl.onrender.com"  # Update after deploying API
    response = requests.post(
        API_URL,
        json={"home_team": home_team, "away_team": away_team, "league_type": league_type}
    )
...
def main():
    ...
    league_name = st.selectbox("Select League", options=list(leagues[category].keys()))
    home_team = st.selectbox("Home Team", options=leagues[category][league_name])
    away_team = st.selectbox("Away Team", options=leagues[category][league_name])

    if st.button("Predict"):
        result = call_api(
            home_team,
            away_team,
            "European" if category == "European Leagues" else "Others"
        )

        # Display prediction
        st.markdown(f'<div class="prediction-result">🏆 {result["prediction"]}</div>', unsafe_allow_html=True)
